...
/* const examples = require('./fontStringsForTesting'); */
...
function findLatinTags(fonts = "") {
  ...
  let languagesRegex = /\/+\*+\s+latin\s+\*+\/+\s*@font-face\s*({[\w\d\s:';\-\/.+,()]+})/gi,
    extractedContent = fonts.match(languagesRegex);
  if (extractedContent) {
    /* remove language tag from result: */
    let removeTagRegex = /@font-face\s*{[\w\s\d\.\*\/,:;'"\-\+()\[\]]+}/gi,
...
function getFonts(allFonts = "") {
  ...
  if (!allFonts) {
    ...
    let fontArray = allFonts.trim().split("@font-face"),
      filteredArray = fontArray.filter((n) => !!n);
    for (let font of filteredArray) {
...
@font-face {
    ...
    font-weight: normal;
    font-display: swap;
    src: url(https://fonts.gstatic.com/s/poppins/v15/pxiByp8kv8JHgFVrLCz7Z1xlFd2JQEk.eot) format('eot');
    unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;
    }
*/

/* MAKE CODE TO OPEN THE NEXT FONT EDITOR AND PRE-FILLS THE NEXT FONT */
/* FONT WEIGHT NOT SAVING AS NUMERICAL */
