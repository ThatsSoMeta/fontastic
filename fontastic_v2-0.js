...
var weightError = document.createElement("p"),
    ...
    base64GuruLink = document.createElement("a"),
    base64BtnGroup = document.createElement("div"),
...
for (var button of [base64Btn, base64GuruLink]) {
    button.style.margin = "5px 0";
    button.style.fontWeight = "bold";
    base64BtnGroup.append(button);
}

base64BtnGroup.classList.add("base64-group");
base64BtnGroup.style.display = "flex";
base64BtnGroup.style.flexDirection = "column";
base64BtnGroup.style.marginLeft = "-60px";

base64GuruLink.innerText = "Base64 Guru";
base64GuruLink.classList.add("fontastic", "guru-link");
base64GuruLink.href = "https://base64.guru/converter/encode";
base64GuruLink.target = "_blank";
base64GuruLink.style.textAlign = "center";
base64GuruLink.style.color = wkndLightBlue;
            ...
            if ($(this).has("label")) {
                if ($(this).children("label").text() == "Family") {
                    ...
                    if (fontList[currentStep] && fontList[currentStep].stack) {
                        $(this)
                            .children("input")
                            .val(fontList[currentStep].stack);
                    ...
                    $(this)
                        ...
                        .on("input", function (e) {
                            stackError.innerText = "";
                            if (
                                !e.target.value.endsWith("serif") &&
                                !e.target.value.endsWith("!important")
                            ) {
                                stackError.innerText =
                                    "Please use a valid font stack.";
                                $stackInput.css("border", "2px solid red");
                            } else {
                    ...
                    $(this).children("label").text() == "Reference URL"
                ) {
                    ...
                    $(this).children("label").text().includes("Font") &&
                    !$(this).children("label").text().includes("Declaration")
                ) {
                    if (!$(".base64_error_text").length) {
                        $(this).append(base64Error);
                    }
                    if (!$(".base64-group").length) {
                        $(this).children("label").after(base64BtnGroup);
                    }
                }
            }
        });
